...
class Settings(BaseSettings):
    ...
    # Tavily settings
    tavily_api_key: Optional[str] = Field(default=None, env="TAVILY_API_KEY", description="Tavily API key for web search")
        ...
        if project_config.exists():
            try:
                ...
                if 'tavily' in config_data:
                    tavily = config_data['tavily']
                    if 'api_key' in tavily:
                        self.tavily_api_key = tavily['api_key']
                    if 'max_results' in tavily:
                        self.tavily_max_results = tavily['max_results']
