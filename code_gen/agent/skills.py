...
class SkillSystem:
    ...
    def load_skills(self):
        """Load skills from directory (supports both flat and nested structure)"""
        if not self.skills_dir.exists():
            return
        
        # 支持两种结构：
        # 1. 扁平结构：skills/*.md
        # 2. 嵌套结构：skills/<skill-name>/SKILL.md
        ...
        for item in self.skills_dir.iterdir():
            if item.is_file() and item.suffix == '.md':
                ...
                if skill_md.exists():
                    self._load_skill_file(skill_md, skill_name=item.name)
